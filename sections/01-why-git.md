Git solves the "project_final_ACTUALLY_FINAL" problem by creating a built-in history of changes. Instead of copying folders, every deliberate save is a **commit** — a snapshot you can always return to.
...
**COMMITS** are deliberate snapshots of your project — like save points in a video game. But git doesn't save a full copy of every file at every snapshot. Instead, it records the <span class="term" tabindex="0">diff<span class="term-tooltip">The difference between two snapshots — which lines were added, which were removed. Git stores diffs, not full copies, which is why it's so efficient.</span></span> — the difference between the last snapshot and the current one.
